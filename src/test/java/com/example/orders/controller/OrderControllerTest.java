...
class OrderControllerTest {
    ...
    void testGetOrders_Success() throws Exception {
        // Generate this test:
        // - Mock orderService.getOrders(...) to return a PagedResponse with:
        //   page=1, limit=10, totalItems=50, totalPages=5, and 1 item OrderResponse(id=1,...)
        // - Perform GET /orders?page=1&limit=10
        // - Expect 200
        // - Assert jsonPath: $.items is array, $.page=1, $.limit=10, $.totalItems=50, $.totalPages=5
        ...
        PagedResponse<OrderResponse> pagedResponse = new PagedResponse<>(
                java.util.List.of(orderResponse),
                1,
                10,
                50,
                5
        );
        Mockito.when(orderService.getOrders(1, 10, null, null, null, null, null))
        ...
        mockMvc.perform(get("/orders")
                ...
                .andExpect(jsonPath("$.items.length()").value(1)) // если мок вернул 1 item
    ...
    @Test
    void testGetOrders_InvalidPage() throws Exception {
        mockMvc.perform(get("/orders")
                        .param("page", "0")
                        .param("limit", "10"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.message").value("Constraint violation"))
                .andExpect(jsonPath("$.details.violations['getOrders.page']")
    ...
    @Test
    void testGetOrders_InvalidLimit() throws Exception {
        mockMvc.perform(get("/orders")
                        .param("page", "1")
                        .param("limit", "101"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.message").value("Constraint violation"))
                .andExpect(jsonPath("$.details.violations['getOrders.limit']")
